...
nn.stochastic_gradient_decent(list(zip(ims, labels)), list(zip(test_ims, test_labels)), 10, 3, 3.0)
#nn = NeuralNetwork(x=np.asarray(x), y=np.asarray(y), hidd_lay=hidd_lay, units_num_in_layer=units_num_in_layer, lerning_rate=2, lmbd=0.001)
## nn.forward_prop()
## nn.back_prop()
#for i in range(20):
#    print('epoch ', i)
...
#    res_tot = []
#    
#    for j in range(len(test_ims)):
#        ind, res = nn.recognize(np.asarray([test_ims[j]]))
#        res_tot.append(res)
#        if ind == test_labels[j]:
#            counter += 1
#    print('res[0] = ', res_tot[0])
#    print("{0} / {1}".format(counter, len(test_ims)))
#    # Update x and y
#    nn.x = deque(np.asarray(x))
#    nn.y = deque(np.asarray(y))
#    # Update deltas
#    for l in range(hidd_lay - 1):
#        nn.delta_w[l] = (np.zeros(shape=(units_num_in_layer, units_num_in_layer)))
#    nn.delta_w[-1] = (np.zeros(shape=(units_num_in_layer, nn.ny)))
#    # Update baies
#    nn.delta_b = list(np.zeros(shape=(hidd_lay, units_num_in_layer)))
#    nn.delta_b.append(np.zeros(shape=(nn.ny)))
